...
| # | 决策项 | 结论 |
|---|---|---|
| D1 | **产品名** | ✅ **已定:灵笺 Lingjian**。仓库两份 docx(心序 Attune)团队定为**仅作参考资料、不维护改名**。吉祥物"雷萌萌"不变 |
...
- [x] **L2-4** 意图补全 → `/suggest`。✅ **代码已就位**:输入任务名时先**秒显本地关键词联想**,**防抖 500ms 后调 AI** 改写成更具体的 2-3 个并替换(提示变"AI 帮你写得更具体"),带序号+内容校验防过期覆盖;无 key/失败保留本地。Playwright 验证(拦截 /suggest 模拟)通过。⏳ 真调待持 key 端验证。(原 C3)
- [x] **L2-5** 周报/归因/状态**文案** → `/insights` + `/report` + `/thermo`。✅ **洞察页归因文案 + 双重报告(效率/心理摘要+归因+建议) + 心力温度计结果解读 都已接 AI**:数字全由前端算法算,文案由 AI 写,失败保留写死、按需缓存;摘要标"· AI 实时生成"。①`/insights`(本周核心洞察)②`/report`(周/月/年/自定义双重报告)③`/thermo`(3 题测完的状态解读)。Playwright 全部验证通过。真调待持 key 端验证。(原 C4)
...
### ⚪ 泳道 4 · 收尾 & 完善(随时)
- [ ] **L4-1** 🧑 答辩稿(团队的 pitch 内容;需要时我可帮起草)。
- [ ] **L4-2** 🧑 录一段完整演示视频 60–90 秒 —— **需团队运行 app 录屏,我做不了**;是提交物之一,保留提醒。
- [ ] **L4-3** 🤖(可选,较难)白板/扫描转任务(多模态)、真语音输入 —— 评估后再决定。(原 F2)
> 注:仓库两份 `.docx`(心序方案 / 想法设计)团队定为**仅作参考资料、不维护、不改名**,不列入任务。
- [x] **L4-4** ✅ **已实现**(游客/正式区分)。`doLogin(via,isGuest)`:**游客登录=保留内置演示任务**(供评委/新人即刻体验);**正式注册**(手机/一键/社交)=`state.isGuest=false` + **首页任务清空**,显示**温暖引导空状态**(「说一句话」按钮 + 3 个示例一键入库 `data-seedadd`)。空状态区分"从没任务的新用户"(欢迎引导)与"完成清空"(庆祝)。Playwright 验证:游客留 16 条、正式清 0、欢迎/入库/删除入口全过。⏳ **待团队定**:目前只清"任务";习惯/成就/洞察周数据仍为演示,是否也清?(注:清了洞察周数据则洞察页无内容可分析)。建议过的"注册=数据归你自己的空间"作为差异卖点,可后续加。

> **真动手时的头三个动作** = **L1-1 → L1-2 → L1-3**(全在浏览器里就能验收,完全不碰 key 和安卓)。

### 🔍 代码级审计(2026-07-22)· app 实际功能盘点
> **方法**:直接对 HTML 逐个功能入口审计(非照 README)。**分发接线完整** —— `openRoute`(10)+`acts`(16)+`onClick` 委托的所有动作都指向已定义的真函数,**无死按钮 / 无未定义引用**。真 AI 功能(解析/拆解/树洞/联想/洞察/报告/温度计,7 接口)全部上线、均带兜底。
>
> **以下是"UI 展示了、但底层是 mock/占位"的功能**(能演示、非真实现;多为原型/套壳环境天然做不了的平台能力,或刻意演示占位):
- [ ] **拍照/扫描转任务**(`openScan`):无真相机/OCR,"用示例拍一张"→写死 3 条。→ 多模态,需视觉模型。
- [ ] **白板涂鸦转任务**(`openSketch`):画布能真手写,但"识别拓扑转任务"是写死结果。→ 多模态。
- [ ] **收集箱 5 个捕获入口**(`openInbox`):锁屏小组件/系统分享/桌面快捷键/浏览器剪藏 —— 仅展示说明,非真集成(需原生/系统能力)。
- [ ] **PDF 导出 / 报告分享链接**(`openReport`):"原型演示"提示,未真生成。
- [ ] **账号绑定 / 一键登录 / 短信验证码**(`openBind`/`openLogin`):mock(任意验证码过、绑定是演示);需 OAuth / 短信网关。
- [ ] (安全·小) 危机词表(13 词)可扩充。(可选·小) 每周心力建议、树洞冷笑话可接 AI。
>
> **结论**:核心"AI 大脑"已全部真;上面这些是**平台集成 / 多模态 / 演示占位**,答辩演示均可正常"演",是否投入真实现按参赛需要评估。真语音识别在安卓 WebView 可能不可用(主用打字)。
...
> 前置:后端跑起来(浏览器开 `http://localhost:8000/health` 应显示 `mode:live`)、前端用 http 打开、已 `git pull` 到最新。建议每条都试"后端在线(真调)"与"后端关掉(兜底)"两面。

**A · 核心 AI 功能(后端在线,应显示绿色/AI 标识)**
- [ ] **说/打一句话出日程**:`+` → 麦克风 → 文字框输入「明天上午写报告，晚上陪我妈散步半小时」→ 发送。期望:顶部绿色「● AI 在线」+ 卡片是你说的事(领域/日期/时长对)。
- [ ] **AI 拆解任务(可编辑)**:点开一个任务 → 「拆成小步」。期望:针对该任务的几步、第一步约 10 分钟;每步可改文字/点时长/删/加;写入用改后的。(用「打扫厨房」这类非套路任务更能看出是真 AI)
- [ ] **雷萌萌树洞**:找雷萌萌聊聊 → 打「今天被批评了好挫败」。期望:温柔具体、能接着上一句聊。
- [ ] **树洞安全(重要)**:打一句含「撑不下去了」之类的话。期望:立刻切「严肃关怀模式」+ 热线 12356、输入栏消失(内容绝不进 AI)。
- [ ] **加任务输入联想**:`+` → 手动填 → 标题框打「写论文」。期望:先秒显本地联想,停顿约 0.5 秒后换成「AI 帮你写得更具体」的三条。
- [ ] **洞察页归因**:洞察 tab →「本周核心洞察」。期望:约 1-2 秒后标题/分析/建议变 AI 写、lead 显示「✦ AI 实时归因分析」、引用真实数字。
- [ ] **双重报告**:洞察 →「双重周报」→ 切效率/心理两个 tab、切周/月/年。期望:摘要/归因/建议是 AI 写、标「· AI 实时生成」。
- [ ] **心力温度计解读**:洞察 →「心力温度计」→ 答 3 题。期望:结果那句解读是 AI 针对你的分数写的。

**B · 兜底测试(把后端 Ctrl+C 关掉,再试上面几个)**
- [ ] 说一句话:显示琥珀「○ 离线示例」、走本地解析、不白屏。
- [ ] 拆解/联想/洞察/报告/温度计:都保留本地默认文案、功能不崩。
- [ ] 关键:全程无白屏、无卡死。

**C · 持久化**
- [ ] 加几个任务 → 刷新页面 → 任务还在。

**D · 已知占位(非 bug,演示时心里有数)**
- [ ] PDF 导出、分享链接是「原型演示」提示(未真生成)。
- [ ] 白板涂鸦转任务、实时语音识别在安卓 WebView 可能不可用(主用打字)。

---

## 15. 进度日志（Changelog,倒序,新的在上）

- **2026-07-22 (更新21)** — 树洞加**在线/离线状态提示**:连上后端 AI → 头部显示"在线 · AI 实时回复";没连上 → "离线 · 本地示例(后端未连上)"。让"为什么回的总是写死那几句"一眼可诊断。团队实测:回复恒为 `BOT_REPLIES`(如"呜,是不是又跟第三章打起来了呀") = 前端没连上 `/chat` = **在跑旧代码**,需 `git pull` + 重启后端 + 浏览器**硬刷新**。Playwright 验证在线/离线两态显示正确。

- **2026-07-22 (更新20)** — 🟠 **L4-4 实现**(游客/正式区分)+ 回应团队测试反馈。①`doLogin(via,isGuest)`:游客=保留内置演示任务;正式(手机/一键/社交)=清空 `state.tasks` + 首页显示**温暖引导空状态**(说一句话按钮 + 3 个示例一键入库 `data-seedadd`);空状态区分"新用户欢迎"与"完成清空庆祝"。Playwright 验证:游客留 16、正式清 0、欢迎/入库/删除入口全过、无报错。②**删除功能一直存在**——点任务→详情浮层→底部"删除（移入回收站）"(`#td-del`→`deleteTask`,移入回收站可恢复);此前团队没找到入口。③团队反馈"树洞/洞察没 AI 反应"**极可能在跑旧代码**:`/chat`·`/insights` 是后加的,需 `git pull` + **重启后端** + 浏览器**硬刷新**;可在 `localhost:8000/docs` 查是否有 `/chat`·`/insights`·`/report`·`/suggest`·`/thermo` 确认后端是否最新。

- **2026-07-22 (更新19)** — 团队决定:仓库两份 `.docx`(心序方案 / 想法设计)**仅作参考资料,不维护、不改名**,从任务清单移除"docx 改名"项(D1 相应更新)。泳道4 现保留:答辩稿(团队)、录演示视频(团队)、多模态转任务(可选/我)。

- **2026-07-22 (更新18)** — 🎉 **泳道2 AI 功能全部收官**。补最后一块:心力温度计 3 题测完的**状态解读接 AI**(后端 `/thermo`:按 3 维分数[心理负荷/焦虑/疲劳]+区间,雷萌萌写个性化解读;前端结果页异步替换预设、失败保留)。Playwright(route 模拟)验证通过。**并做了一次全代码审计**:README §6 的"假 AI"已全部接真(7 个 AI 接口);审计发现的零星待评估项(每周建议/冷笑话可选接 AI、PDF导出/分享是演示占位、危机词可扩充)记入 §11 审计补充。新增 **§16 手动测试清单**。

- **2026-07-22 (更新17)** — 🟡 **L2-5 延伸:双重报告(`openReport`)文案接 AI**。新增后端 `/report`:输入某区间(周/月/年/自定义)真实数字,AI 输出 `{sumA 效率摘要, sumB 心理摘要, attr 归因, suggest 建议}`。前端用"合并数据源 + 按区间缓存(`_aiRep`) + 异步回来重绘"接入:有 AI 用 AI、无则用写死;摘要标"· AI 实时生成";切 tab/区间不重复请求;浮层关闭后不再重绘。Playwright(route 模拟)验证:效率 tab sumA、心理 tab sumB/归因/建议均替换、无报错。**至此泳道2 的 AI 文案基本齐**(解析/拆解/树洞/联想/洞察/双重报告),仅剩压力校准(小)。真调待持 key 端验证。

- **2026-07-22 (更新16)** — 🟡 **L2-4 意图补全 接 AI**。新增后端 `/suggest`:把用户正在输入的任务名改写成 2-3 个更具体、可执行的写法(JSON 模式)。前端加任务输入框:先**秒显本地关键词联想**(零延迟),**防抖 500ms** 后调 `/suggest` 用 AI 结果替换(提示变"AI 帮你写得更具体"),用序号+内容双校验**防过期结果覆盖**新输入;无 key/失败保留本地联想。Playwright(route 拦截模拟 AI)验证:本地即时→AI 替换、无报错。真调待持 key 端验证。

- **2026-07-22 (更新15)** — 两个决策记录。①**打包时机**:团队决定**先把功能全做完再统一打包**(避免反复 cap sync/重打浪费),🔵 泳道3 推后;继续按泳道2 任务顺序推进,后期统一测试所有功能。②**L4-4 方向明确**:游客=进去加载内置演示任务(供体验/演示),正式注册=首页空白自己添加,**二者功能不做差异**。接力窗口建议(见 §11 L4-4):给正式用户温暖空状态 + 给注册一个真实好处(数据归属),避免"注册=空白页"劝退与"无理由注册"。待团队定夺后再实现。
