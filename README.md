...
- **当前状态**: 方案已完善。**决定所有功能都真做**(时间充裕、无截止日,目标是把作品做完整做好),按 §11 任务表增量推进。**尚未开始写后端/功能代码。** 下一步 = §11 阶段 A(先申请 DeepSeek key)。
...
## 11. 任务分解 & 干净交接节点（Checkpoints）

> **决策**:所有功能都真做,**增量式推进** —— 每完成一步都停在一个"能提交、能演示、能转手"的干净节点。任一节点都可交接给另一个人接力。
> **顺序**:先地基(后端 + 第一条真链路)→ 逐个把"假 AI"变真 → 持久化 → 打包 → 收尾。**每个任务做完即 commit&push + 在此打勾。**

### 阶段 A · 准备(前置,先做)
- [ ] **A1** DeepSeek 实名注册 + 充值 + 拿 key,存后端环境变量。(阻塞后端,最先做)
- [ ] **A2** 装环境:Node 22+ / Android Studio(打包时才需要;纯开发先用浏览器)。
- [ ] **A3** 建 Capacitor 项目骨架:HTML 放 `www/index.html` + 字体离线内嵌(§9)。

### 阶段 B · 后端地基 + 第一条真链路 → 🏁【干净节点①:说一句话真出日程】
- [ ] **B1** 搭最小 FastAPI(含 `/health`)。
- [ ] **B2** `/parse` 接 DeepSeek,输出**严格对齐 §7 的 9 字段**(先用假后端联调形状,再接真 DeepSeek)。
- [ ] **B3** 前端 `renderResult`(HTML 第3541行)加 try后端/catch本地兜底 + "AI在线/离线"徽章(§10)。
- [ ] **B4** 浏览器里跑通:说一句话→真调→出日程。🏁

### 阶段 C · 逐个把"假 AI"变真(每个功能一个干净节点)
- [ ] **C1** AI 拆解任务(`chainSteps` → `/decompose`)。🏁
- [ ] **C2** 雷萌萌树洞对话(→ `/chat`,含危机词硬过滤)。🏁
- [ ] **C3** 意图补全 / 语义解析增强。🏁
- [ ] **C4** 压力评估 + 周报/归因**文案**(数字用算法算,文案用 LLM)。🏁

### 阶段 D · 持久化(让刷新不丢)
- [ ] **D1** 先加前端 `localStorage`(最省事);数据量大再考虑后端数据库。🏁
- [ ] **D2** 时间盘/热力图/周报喂真数据。🏁

### 阶段 E · 打包成 Android app
- [ ] **E1** `cap add android` + `cap sync`;处理明文流量(HTTPS 隧道或 cleartext 配置,§9)。
- [ ] **E2** 模拟器/真机验证核心链路联后端;图标/名字/签名。🏁【可安装 apk】

### 阶段 F · 完善 & 收尾
- [ ] **F1** 方案 docx 改名 心序→灵笺 + 答辩稿。
- [ ] **F2**(可选,较难)白板转任务(多模态)、真语音输入 —— 评估后再决定做不做。
- [ ] **F3** 录一段完整演示视频。
