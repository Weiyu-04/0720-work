...
- [x] **L2-5** 周报/归因**文案** → `/insights` + `/report`。✅ **洞察页"本周核心洞察"归因文案 + 双重报告(`openReport`)叙事文案都已接 AI**:①`/insights` 输入本周真实数字→ `{headline, analysis[], suggestions[]}`;②`/report` 输入某区间(周/月/年/自定义)真实数字→ `{sumA 效率摘要, sumB 心理摘要, attr 归因, suggest 建议}`。前端均"数字算法算、文案 AI 写",按区间缓存、异步回来重绘、失败保留写死;压力值用真实 `avgStress`;摘要标"· AI 实时生成"。Playwright 验证通过。⏳ 只剩压力校准结果文案(小)。真调待持 key 端验证。(原 C4)
