...
- [ ] **L4-4** 🆕 **游客 / 正式用户区分**(演示要体现差别)。**现状**:二者功能完全一样(都 `loggedIn=true`+`free`,HTML 4575/4581)。**团队定的方向(2026-07-22)**:**功能不做差异**;区别只在初始数据 —— **游客进去 = 加载内置演示任务**(给评委/新人即刻体验),**正式注册 = 首页空白**、自己添加任务。**接力窗口的建议(待团队定夺)**:方向可行、且很利于演示(游客一键看到丰满的 app);但两点建议 —— ①别让正式用户落到"冷冰冰空白页":给温暖空状态(欢迎语 + 2-3 个"说一句话"起步示例/按钮),避免经典"空状态劝退";②给"注册"一个真实好处(如:数据存进你自己的空间、游客数据是临时演示),否则"注册=失去演示内容"缺乏动力,也呼应"隐私优先·数据归你"主张。Pro(¥18/月)是另一条独立轴,不在此讨论。
