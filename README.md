...
- **最后进展**: 🟡 **泳道2 核心真链路已在真 app 验证通过**(2026-07-22):说/打一句话 → 真调 DeepSeek(v4-flash) → 出日程,团队本地 key + 前端 http 端到端跑通(绿色"AI在线"徽章 + 真解析你说的内容)。泳道1(假后端 / try后端catch本地降级 / 徽章 / localStorage 持久化 / 打字输入)已全部完成。**决定所有功能都真做**,按 §11 增量推进。下一步 = 泳道2 其余"假AI变真"(L2-2 拆解 / L2-3 树洞 / L2-5 周报文案)择一,或并行做泳道3 打包准备。
...
- [x] **L1-5** 打字输入:语音浮层加文字框(回车/发送),走**同一条 `renderResult` 链路**(后端/本地兜底/徽章全复用)。✅ 落实 §13"打字为安卓 WebView 主力入口"。

### 🟡 泳道 2 · 等 key(到手后第一步只是"一行切换")—— key 非阻塞、可随时并行申请
- [x] **L2-0** DeepSeek 拿 key。✅ 2026-07-22 已拿到 key。**下一步(人工)**:把 key 写进 `backend/.env`(`DEEPSEEK_API_KEY=...`)→ 重启后端 → `curl /health` 应显示 `mode:live`。⚠️ key 绝不进 chat/git/apk。
- [x] **L2-1** 真调 DeepSeek。✅ **已验证通过**(2026-07-22 团队用真 key 实测:"后天下午两点交课程作业,今晚十点前回导师邮件"→ 正确拆成 2 件事,domain/day/time 全对)。`real_parse()`(v4-flash、JSON 模式、注入北京时间、空返回重试)+ `/parse` 自动切换 + 9 字段校验 + `_norm` 兜底(est=0→30、load 由 est 推、restorative 由 domain 定)。**前端不用改。**🏁
- [x] **L2-2** AI 拆解任务 → `/decompose`。✅ **代码已就位**:后端 `/decompose` 两种模式(`phases` 大目标拆 5-8 阶段 / `steps` 拆 3-4 个破冰小步,第一步~10min);前端"AI 帮我拆解"(`chainSteps`)+"拆成可执行小步"(`openSplit`)都接上,加载态 + 后端不可达/空则回退本地模板。兜底路径 Playwright 验证通过。⏳ 真调待持 key 端验证。(原 C1)
- [x] **L2-3** 雷萌萌树洞对话 → `/chat`。✅ **代码已就位**:后端 `/chat`(人设=先共情不说教/简短温柔,带最近几轮上下文)+ **双层危机词硬过滤**(前端先拦 + 后端再拦,命中即切"严肃关怀模式"热线、**绝不进 AI**);前端 `reply()` try后端/catch本地 `BOT_REPLIES` 兜底 + 对话历史 + AI 回复 HTML 转义。Playwright 验证:普通消息本地兜底出气泡、危机词切热线模式(输入栏隐藏)、无报错。⏳ 真调待持 key 端验证。(原 C2)
- [ ] **L2-4** 意图补全 / 语义解析增强。🏁(原 C3)
- [x] **L2-5** 周报/归因**文案** → `/insights`(核心完成)。✅ **洞察页"本周核心洞察"归因文案已接 AI**:后端 `/insights` 输入前端算好的本周真实数字(压力/恢复占比/焦虑趋势/黄金时段/领域占比/透支),输出 `{headline, analysis[], suggestions[]}`;前端 `computeWeekStats()` 算数字、`RenderInsights` 后异步填充、缓存、失败保留写死默认;压力值用真实 `avgStress`。Playwright 验证通过。⏳ 待续:双重周报(`openReport`)文案、压力校准结果同法接入;真调待持 key 端验证。(原 C4)
...
- [ ] **L4-4** 🆕 **游客 / 正式用户权益区分**(演示要体现差别)。**现状核实(2026-07-22)**:跳过注册(游客试用)与正式注册**功能完全一样** —— 两条路都只是 `loggedIn=true`+`membership='free'`(HTML 4575/4581);连 free/pro 差异目前也只是升级卡文案、**未真正强制**。**要做**:让"注册 / 升级"解锁看得见的东西,演示能当场对比。**候选分档(待团队拍板具体边界)**:游客=试用核心解析,但 ①数据不保证留存(换设备/清缓存即丢)②AI 解析/拆解每日限 N 次 ③洞察/周报/导出/全部装扮上锁 + 处处"注册解锁"引导;正式(free)=数据入账号 + 额度更高 + 解锁洞察周报;Pro=无限解析 / 双报告 / 全装扮 / 多端同步。
...
- **2026-07-22 (更新14)** — 🟡 **L2-5 洞察归因文案 接 AI(核心完成)**。新增后端 `/insights`:输入前端算好的本周真实数字(平均压力/恢复性占比/焦虑趋势/时间最多领域及占比/黄金时段及效率/透支点),AI 输出 `{headline, analysis[], suggestions[]}`(JSON 模式、temperature 0.6)。前端 `computeWeekStats()` 从 `state.week`+`focusLog` 算数字;`RenderInsights` 渲染后异步 `refreshInsight()` 填充"本周核心洞察"块,结果缓存 `_aiInsight`(切页不重复请求)、失败保留原写死文案;压力值 stat 改用真实 `avgStress`;AI 文案 HTML 转义。Playwright 验证:数字计算正确、AI 填充逻辑正常、无报错。⏳ 双重周报(`openReport`)文案、压力校准结果同法待续;真调待持 key 端验证。

- **2026-07-22 (更新13)** — 🟡 **L2-3 雷萌萌树洞对话 代码就位**。新增后端 `/chat`:雷萌萌人设(先共情、不说教、简短温柔)、带最近几轮上下文、temperature 0.8。**安全为重**:危机词**双层硬过滤**(前端 `reply()` 先查 `CRISIS_WORDS`、后端 `/chat` 再查),命中即**绝不进模型**、直接切"严肃关怀模式"(全国心理援助热线 12356 等),输入栏隐藏。前端 `reply()` 改 async:try 后端 → 空/失败回退本地 `BOT_REPLIES`,维护对话历史;AI 回复做 HTML 转义防注入。Playwright 验证:普通消息兜底出气泡、危机词切热线模式、无 JS 报错。真调待持 key 端验证。

- **2026-07-22 (更新12)** — 团队提新需求,记入 §11 **L4-4**:**区分游客(跳过注册)与正式用户的权益**,演示要体现差别。核实现状:目前二者功能**完全一样**(都 `loggedIn=true`、`membership='free'`),free/pro 差异也未真正强制。已记录候选三档方案(游客 / 正式 free / Pro),具体权益边界待团队拍板。

- **2026-07-22 (更新11)** — L2-2 体验补强(团队实测反馈:AI 拆的步骤应能改,"不是所有人打扫厨房的步骤都一样")。"拆成可执行小步"(`openSplit`)现在**每步可编辑**:改标题(输入框)、点时长循环调整(10/15/20/30/45/60/90 分)、× 删除、"＋ 加一步";写入任务库时用编辑后的内容并过滤空步。Playwright 验证:改标题→写入用改后的文字、加/删/调时长全部通过,无 JS 报错。(大目标"AI 帮我拆解"页本就支持编辑。)

- **2026-07-22 (更新10)** — 🟡 **L2-2 AI 拆解任务 代码就位**。新增后端 `/decompose`(有 key 走真调、无 key/失败返回空数组):两种模式 —— `phases`(大目标→5-8 阶段)、`steps`(具体任务→3-4 个可执行小步,第一步~10min 破冰)。前端两处接上并保留本地兜底:①"拆解任务"页"AI 帮我拆解"(`chainSteps` 改 async + 拆解中态);②任务详情"拆成可执行小步"(`openSplit` 改 async,先弹加载态、AI 回来再填充)。统一用 `aiDecompose(title,mode)` 做 try后端/catch本地。Playwright 验证兜底路径:openSplit 出 3 步(破冰10min)、chainBuilder 出 8 阶段,无 JS 报错。真调待持 key 端验证。

- **2026-07-22 (更新9)** — 🟡 **L2-1 真调 DeepSeek 验证通过**,泳道2 第一条真链路(说话→真调→出日程)全线打通。团队本地放 key(`backend/.env`)、`/health` 显示 mode=live、`/parse` 用真句子测通(正确拆成 2 件事、domain/day/time 准确)。**实测暴露并修复一个 bug**:模型对未说时长的任务返回 `est=0`(前端会显示"约0分钟")→ ①提示词要求"没说时长就合理估计、别返回0";②后端 `_norm()` 兜底(est 兜成正整数默认30、load 由 est 推、restorative 由 domain 定、conf 兜底),保证字段自洽。

- **2026-07-22 (更新8)** — 泳道2 L2-1 **代码就位**。后端 `real_parse()` 按 §8 写好(v4-flash、JSON 模式、注入北京时间、空返回重试、9 字段校验);`/parse` **自动切换** —— 有 `DEEPSEEK_API_KEY` 走真调、无 key 走假后端;`/health` 报 `mode:live/fake` 便于确认 key 是否加载。用 python-dotenv 自动读 `backend/.env`。**模型确认用最便宜的 v4-flash**(联网核实 2026-07:V4 Flash ≈ $0.14/$0.28 每百万 in/out token、命中缓存输入 $0.003,新账号 500 万 token 免费额度;V4 Pro 贵约 3 倍,不用)。in-process 验证通过(导入 / fake / `{tasks:[...]}` 解析 / 校验丢弃非法 domain)。**真实 DeepSeek 调用待持 key 的机器上验证** —— 团队已拿到 key,按 `backend/README.md` 放进 `.env` 即自动生效。安全:key 只进 `backend/.env`(已 .gitignore),绝不进 chat/git/apk。

- **2026-07-22 (更新7)** — 泳道1 补强:语音浮层新增**打字输入**(文字框 + 发送/回车),复用 `renderResult` 同一条链路(后端→本地兜底→在线/离线徽章)。落实 §13"打字为安卓 WebView 主力入口"(Web 语音在安卓 WebView 常不可用)。Playwright 验证:打字→发送→出 3 卡 + "● AI 在线"徽章,无报错。
