...
- **当前状态**: 方案已完善。**决定所有功能都真做**(时间充裕、无截止日,目标是把作品做完整做好),按 §11 任务表增量推进。**尚未开始写后端/功能代码。** 下一步 = §11 **泳道1**(用假后端把整条链路 + localStorage 在浏览器跑通,**无需 key、无需安卓**;DeepSeek key 可并行申请、**非阻塞**)。
...
- [ ] **L1-1** 搭最小 FastAPI:`/health` + `/parse`**先返回写死的 9 字段示例数组**(§7 的假后端),让前端 fetch 先把形状打通。(原 B1 + B2 前半)
- [ ] **L1-2** 前端 `renderResult`(HTML 第3541行)改 **try 后端 / catch 回退本地 `parseUtterance`**(8秒超时)+ "AI在线/离线"徽章(§10)。(原 B3)
- [ ] **L1-3** **localStorage 持久化**:`state.tasks` 等存取,刷新不丢(原 D1,提前到这里 —— 纯前端、价值高、直接修掉"刷新即清空"⚠️)。
- [ ] **L1-4** 浏览器里端到端验收:说一句话→(走假后端)出日程→改/存→刷新还在。🏁 **一个对 key、安卓零依赖的可演示里程碑。**
