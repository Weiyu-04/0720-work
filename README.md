...
- [ ] **L2-1** 真调 DeepSeek。✅ **代码已就位**:`backend/main.py` 的 `real_parse()`(v4-flash、JSON 模式、注入北京时间、空返回重试、9 字段校验);`/parse` **自动切换**(有 key 走真调、无 key 走假后端)。in-process 已验证(导入/fake/JSON解析/校验)。⏳ **待办**:持 key 的机器上跑一句"跟示例不同"的话,确认真解析了你说的内容(徽章转绿 + 卡片是你说的事)。**前端不用改。**🏁
